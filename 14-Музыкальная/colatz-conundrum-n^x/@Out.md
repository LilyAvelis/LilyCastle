...
### Уточнение границ (Boundaries Check)

Проведена проверка границ "Света" и "Смерти".

1.  **Линейный случай (d=1):**
    - Границы "Света": **[-1.97, 2.07]**.
    - Это подтверждает теоретическую оценку |a| < 2 (с небольшим запасом из-за дискретности).
    - "Столб смерти" начинается за пределами |a| > 2.

2.  **Квадратичный случай (d=2):**
    - Ширина "Канала Света" по k2: **[0.0045, 0.0719]**.
    - Это подтверждает, что "Свет" существует только в узком диапазоне, близком к нулю.
    - При |k2| > 0.1 (и тем более > 1) наступает гарантированная "Смерть".

**Итог:**
- Линейный мир стабилен в широком диапазоне (|a| < 2).
- Квадратичный мир стабилен только в узкой щели (k2 ~ 0), имитирующей линейность.
- "Столб смерти" для d=2 действительно начинается очень рано (уже при k2 > 0.07).
